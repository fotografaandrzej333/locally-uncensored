...
Drop-in hotfix on top of v2.4.4. Six bugs total: five user-reported (A — Discord/Reddit; B — GH #38; C — GH Discussion #39; D — Discord; E — GH #32 comment) plus one discovered during the Arch live verification sweep (F).
...
### Tests
- `vitest`: 2271 passed (93 files) — +7 new tests for `parseOllamaError` missing-blob coverage in `src/lib/__tests__/ollama-errors.test.ts` (Anson192 verbatim error, generic no-fallback-model, forward+back-slash tolerance, Rust-proxy wrapping, chat-style message wording with + without model), +1 smoke test for the new `videohelpersuite` `CUSTOM_NODE_REGISTRY` entry in `src/api/__tests__/comfyui-bundles.test.ts`.
- `cargo test --release`: 70 passed — +5 new tests in `src-tauri/src/main.rs::tests` for the Bug D Linux webkit workaround function (`webkit_workaround_sets_both_vars_when_unset`, `preserves_user_dmabuf_override`, `preserves_user_compositing_override`, `preserves_empty_string_as_explicit_unset`, `is_idempotent`) plus +13 new tests for Bug E in `src-tauri/src/commands/install.rs::tests` and `src-tauri/src/python.rs::tests` (`is_pep668_protected_returns_false_for_empty_bin`, `is_pep668_protected_returns_false_for_garbage_bin`, `diagnose_externally_managed_mentions_venv`, `diagnose_externally_managed_includes_distro_install_commands`, `diagnose_externally_managed_alt_format_matches`, `transient_rejects_externally_managed`, `venv_python_path_matches_platform_layout`, `venv_python_path_is_under_comfyui_dir`, `resolve_returns_none_when_venv_missing`, `resolve_returns_some_when_venv_python_exists`, plus 3 baseline `is_real_python` tests promoted from inline assertions). Bug D tests serialize via `ENV_MUTEX` since env vars are process-global.
